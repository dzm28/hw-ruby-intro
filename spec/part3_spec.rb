...
describe 'BookInStock' do
  ...
  describe 'getters and setters' do
    ...
    it 'sets ISBN [1 points]', points: 1 do
      expect(@book.isbn).to eq('isbn1')
    end

    it 'sets price [1 points]', points: 1 do
      expect(@book.price).to eq(33.8)
    end

    it 'is able to change ISBN [1 points]', points: 1 do
      @book.isbn = 'isbn2'
      expect(@book.isbn).to eq('isbn2')
    end

    it 'is able to change price [1 points]', points: 1 do
      @book.price = 300.0
  ...
  describe 'constructor' do
    it 'rejects invalid ISBN number [1 points]', points: 1 do
      expect { BookInStock.new('', 25.00) }.to raise_error(ArgumentError)
    end

    it 'rejects zero price [1 points]', points: 1 do
      expect { BookInStock.new('isbn1', 0) }.to raise_error(ArgumentError)
    end

    it 'rejects negative price [1 points]', points: 1 do
  ...
  describe '#price_as_string' do
    ...
    it 'displays 33.95 as "$33.95" [1 points]', points: 1 do
      expect(BookInStock.new('isbn11', 33.95).price_as_string).to eq('$33.95')
    end

    it 'displays 1.1 as $1.10 [1 points]', points: 1 do
      expect(BookInStock.new('isbn11', 1.1).price_as_string).to eq('$1.10')
    end

    it 'displays 20 as $20.00 [1 points]', points: 1 do
